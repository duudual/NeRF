...
## dynamic nerf
本文测试了两种实现方式，一种是直接将t经过类似于位置的嵌入后同样加入到MLP中进行预测，另一种是使用两个神经网络，一个是预测静态内容，一个是预测相对于静态的偏差，两个相结合，预测出任意时刻的场景内容。
两种方法都要注意weight decay的使用，否则容易出现只预测出白色图片而无法优化的情形
...
从结果可以看到，传统的dynamic NeRF的实现上，不论是直接对t进行嵌入还是动态静态双MLP，都会出现跳动时模糊的情况，这是使用“回归”手段解决升维问题的必然结果。而使用我新提出的特征插值方法，不仅保留了静态部分的稳定，更对动态部分有了准确而合理的预测，预测结果更清晰、更准确。
同时更有意义的是，由于基于ViT模型，特征具有语义特征，这意味着我们不是像传统方案一样对单个3D点获得其t时刻的位置或静态位置与位移，而是在考虑整体语义的同时，预测其t时刻的实际位置。举例而言，在lego场景中，当预测运动的挖铲上的点的位置时，会考虑整体这是一个挖铲的语义信息，从而在得到实际位置时，会综合考虑这一part的运动情况，避免了实际相邻的点由于单独预测而产生的预测位置相差甚远的情况。
