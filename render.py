...
from utils import DEBUG, to8b
from rays import get_rays, ndc_rays
from sampling import sample_pdf
...
def batchify_rays(rays_flat, chunk=1024*32, **kwargs):
    ...
    Args:
        ...
        chunk: chunk size
        **kwargs: arguments to pass to render_rays
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    # Ensure tensor is on the same device as dists
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    Args:
        render_poses: [N, 3, 4] camera poses to render
        hwf: [H, W, focal] height, width, focal length
        K: camera intrinsic matrix
        chunk: chunk size for rendering
        render_kwargs: arguments to pass to render
        gt_imgs: ground truth images (optional)
        savedir: directory to save rendered images (optional)
        render_factor: downsampling factor for rendering
    
    Returns:
        rgbs: [N, H, W, 3] rendered RGB images
        disps: [N, H, W] rendered disparity maps
    """
    H, W, focal = hwf
